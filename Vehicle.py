def main():
    ...
    text_no = text_no.replace(" ", "");  #remove white spaces
    text_no = text_no.upper();      # capitalize the letters


    print(text_no)
    myclient = pymongo.MongoClient("mongodb://localhost:27017/*") # mongo object connection
    mydb = myclient["gfg"] #database
    mycol = mydb["details"] #collection
    mycol1 = mydb["loginfo"] #2_nd_collection
    ...
    for x in mycol.find({"vehicleno":text_no},{"_id": 0, "name": 1, "type": 1}):
        print("Allowed to enter")
        flag = 1;
        print(x)
        loginfo = {"vehicleno": text_no, "time": localtime}
        y = mycol1.insert_one(loginfo)
        print(localtime)
